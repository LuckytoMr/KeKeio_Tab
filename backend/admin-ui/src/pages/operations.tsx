...
const emptySMTP: SMTPFormState = { provider: defaultSMTPPreset.id, host: defaultSMTPPreset.host, port: defaultSMTPPreset.port, tls: defaultSMTPPreset.tls, from: defaultSMTPPreset.defaultFrom ?? "", username: defaultSMTPPreset.username ?? "", password: "", recipient: "", passwordConfigured: false };
...
export function SystemAreaPage({ client, route, notify }: { client: ApiClient; route: AdminRoute; notify: (message: string, tone?: string) => void }) {
  const config = getOperationConfig(route.page); const resource = useRouteData<SystemPayload>((signal) => client.get(config.endpoint, signal), [client, config.endpoint]); const [settings, setSettings] = useState<Record<string, string | boolean>>({}); const [smtp, setSMTP] = useState<SMTPFormState>(emptySMTP); const smtpRef = useRef<SMTPFormState>(emptySMTP); const smtpConfiguredRef = useRef(false); const smtpDirtyRef = useRef(false); const [smtpVerified, setSMTPVerified] = useState(false); const [busy, setBusy] = useState(false); const smtpPreset = getSMTPPreset(smtp.provider);
  useEffect(() => { if (resource.data?.settings) { setSettings(toEditableSettings(resource.data.settings)); const smtpValue = resource.data.settings.smtp; const nextSMTP = toSMTPForm(smtpValue); smtpRef.current = nextSMTP; smtpConfiguredRef.current = hasSMTPConfiguration(smtpValue); smtpDirtyRef.current = false; setSMTP(nextSMTP); setSMTPVerified(Boolean((smtpValue as Record<string, unknown> | null)?.verified)); } }, [resource.data]);
  const patchSMTP = <K extends keyof SMTPFormState>(key: K, value: SMTPFormState[K]) => { const next = { ...smtpRef.current, [key]: value, ...((key === "host" || key === "port" || key === "tls") ? { provider: "custom" as const } : {}) }; smtpRef.current = next; setSMTP(next); if (key !== "recipient" && key !== "provider") { smtpDirtyRef.current = true; setSMTPVerified(false); } };
  const chooseSMTPProvider = (provider: SMTPProviderId) => {
    ...
    const previousPreset = getSMTPPreset(smtpRef.current.provider);
    const username = preset.username ?? (previousPreset.username && smtpRef.current.username === previousPreset.username ? "" : smtpRef.current.username);
    const next: SMTPFormState = { ...smtpRef.current, provider, host: preset.host, port: preset.port, tls: preset.tls, username };
    smtpRef.current = next;
    smtpDirtyRef.current = true;
    setSMTP(next);
  ...
  const save = async (event: Event) => { event.preventDefault(); if (Boolean(settings.registrationEnabled) && !smtpVerified) { notify("开放注册前必须使用当前 SMTP 配置成功发送测试邮件", "error"); return; } const includeSMTP = smtpConfiguredRef.current || (smtpDirtyRef.current && (smtp.password.trim() !== "" || smtp.username.trim() === "")); setBusy(true); try { await client.put("/api/admin/v1/system/settings", editableSettingsPayload(settings, smtp, includeSMTP)); notify("安全设置已保存"); resource.retry(); } catch (error) { notify(error instanceof Error ? error.message : "保存失败", "error"); } finally { setBusy(false); } };
...
function hasSMTPConfiguration(value: unknown): boolean { return Boolean(value && typeof value === "object" && String((value as Record<string, unknown>).host || "").trim()); }
...
function editableSettingsPayload(value: Record<string, string | boolean>, smtp?: SMTPFormState, includeSMTP = true) { const payload: Record<string, unknown> = { registrationEnabled: Boolean(value.registrationEnabled), publicBaseUrl: String(value.publicBaseUrl || ""), webOrigins: String(value.webOrigins || ""), maxUsers: String(value.maxUsers || ""), profileKiB: String(value.profileKiB || ""), storageGiB: String(value.storageGiB || ""), versionsPerUser: String(value.versionsPerUser || ""), accessLogDays: String(value.accessLogDays || ""), auditLogDays: String(value.auditLogDays || "") }; if (includeSMTP && smtp && (smtp.host || smtp.from || smtp.username || smtp.passwordConfigured || smtp.password)) payload.smtp = smtpPayload(smtp); return payload; }
