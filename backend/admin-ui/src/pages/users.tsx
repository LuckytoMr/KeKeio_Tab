...
interface UserListItem { id: string; email: string; displayName?: string; status: string; verificationStatus?: string; lastActivityAt?: string; deviceCount?: number; createdAt?: string }
...
export function UsersPage({ client, route, onNavigate }: { client: ApiClient; route: AdminRoute; onNavigate: (href: string) => void }) {
  ...
  return <>
    <PageHeading eyebrow="用户与同步" title="用户" description="搜索账号、查看验证状态和设备数量。完整配置默认保持隐藏。" />
    ...
    {resource.data ? resource.data.items.length ? <div class="data-surface"><table class="responsive-table"><thead><tr><th scope="col">用户</th><th scope="col">状态</th><th scope="col" class="optional-column">最后活动</th><th scope="col" class="optional-column">设备</th><th scope="col"><span class="sr-only">操作</span></th></tr></thead><tbody>{resource.data.items.map((item) => {
      ...
      return <><tr key={item.id}><td><div class="identity-cell"><span class="avatar" aria-hidden="true"><UserRound size={17} /></span><div><a class="primary-link" href={href} aria-label={`查看 ${item.email}`} onClick={(event) => { event.preventDefault(); onNavigate(href); }}>{item.email}</a><small>{item.displayName || item.verificationStatus || "插件用户"}</small></div></div></td><td><StatusBadge value={item.status} /></td><td class="optional-column">{formatDate(item.lastActivityAt)}</td><td class="optional-column">{item.deviceCount ?? 0}</td><td><ExpandButton expanded={isExpanded} controls={detailId} onClick={() => setExpanded(isExpanded ? null : item.id)} label={`${item.email} 的更多字段`} /></td></tr>{isExpanded ? <tr id={detailId} class="expanded-row"><td colSpan={5}><dl><div><dt>验证状态</dt><dd>{item.verificationStatus || "—"}</dd></div><div><dt>创建时间</dt><dd>{formatDate(item.createdAt)}</dd></div><div><dt>设备数</dt><dd>{item.deviceCount ?? 0}</dd></div></dl></td></tr> : null}</>;
...
interface UserDetailData { user: UserListItem; sessions?: Array<{ id: string; deviceName?: string; createdAt?: string; lastUsedAt?: string }>; profile?: { version?: number; schemaVersion?: number; bytes?: number; groups?: number; shortcuts?: number }; attempts?: Array<{ id: string; status: string; code?: string; requestId?: string; createdAt?: string }>; versions?: ProfileVersionItem[] }
...
export function UserDetailPage({ client, route, onNavigate, notify }: { client: ApiClient; route: AdminRoute; onNavigate: (href: string) => void; notify: (message: string, tone?: string) => void }) {
  ...
  return <>
    ...
    <div class="detail-sections"><section><h2>账号与配置</h2><dl class="definition-grid"><div><dt>状态</dt><dd><StatusBadge value={data.user.status} /></dd></div><div><dt>验证</dt><dd>{data.user.verificationStatus || "—"}</dd></div><div><dt>当前版本</dt><dd>{data.profile?.version ?? "—"}</dd></div><div><dt>Schema</dt><dd>{data.profile?.schemaVersion ?? "—"}</dd></div><div><dt>Profile 大小</dt><dd>{formatBytes(data.profile?.bytes)}</dd></div><div><dt>分组 / 快捷方式</dt><dd>{data.profile?.groups ?? 0} / {data.profile?.shortcuts ?? 0}</dd></div></dl></section><section><h2><MonitorSmartphone size={18} aria-hidden="true" />活跃设备与会话</h2>{data.sessions?.length ? <ul class="action-list">{data.sessions.map((session) => <li key={session.id}><div><strong>{session.deviceName || "未命名设备"}</strong><p>最近使用 {formatDate(session.lastUsedAt)}</p></div><button class="button secondary" type="button" onClick={() => setConfirm({ action: "revoke", target: session.id })}>撤销会话</button></li>)}</ul> : <EmptyState title="没有活跃会话" description="用户下次登录后会创建新会话。" />}</section><section><h2><KeyRound size={18} aria-hidden="true" />最近同步</h2>{data.attempts?.length ? <ul class="event-list">{data.attempts.map((attempt) => <li key={attempt.id}><StatusBadge value={attempt.status} /><div><strong>{attempt.code || "同步完成"}</strong><p>{formatDate(attempt.createdAt)} · request {attempt.requestId || "—"}</p></div></li>)}</ul> : <EmptyState title="没有同步尝试" description="设备首次同步后会显示诊断记录。" />}</section><section><h2><History size={18} aria-hidden="true" />配置版本</h2>{data.versions?.length ? <ul class="action-list">{data.versions.map((version) => <li key={version.id}><div><strong>版本 {version.version}</strong><p>{formatVersionStructure(version)}</p><p>{formatVersionChanges(version)}</p></div><button class="button secondary" type="button" aria-label={`恢复版本 ${version.version}`} onClick={() => setConfirm({ action: "restore", version })}>恢复此版本</button></li>)}</ul> : <EmptyState title="没有历史版本" description="服务器确认首次配置后会产生版本。" />}</section></div>
