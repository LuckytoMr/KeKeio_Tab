...
外部仍是标准 `80/443`，因此访问 `tab.kekeio.com` 不需要添加端口；外网根地址返回后端 liveness 状态，允许的局域网来源会跳转到后台。路由器可以把外部 `80/443` 转到 Docker 主机的 `8080/8443`；`8881` 只是后端内部/上游端口，绝不能直接做 WAN 转发。DNS 只负责域名解析，不负责端口转换。
