...
- 部署：路由器通过 Docker Compose 运行后端与 Caddy；后端内部端口固定为 `9009`，公网只开放标准 `80/443`，安装页和后台只允许本机/局域网。
...
生产流量固定经过 `tab.kekeio.com:443 → Caddy:443 → backend:9009`。路由器可把 WAN `80/443` 转换到 Docker 主机高位端口 `8080/8443`，但不得把 WAN `9009` 直接转给后端。Caddy 对公网只放行根路径在线状态、插件 API、账号页及其静态资源和健康端点；管理/安装路由要求私网来源，其他路径返回 404。
